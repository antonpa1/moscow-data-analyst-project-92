--customers_count
select
    count(customer_id) as customers_count  -- счтиаем кол-во всех id
from customers;
...
from sales as s
    ...
    join products as p
        on s.product_id = p.product_id  -- объединяем данные из таблиц
group by
    e.employee_id,
    e.first_name,
    e.last_name  --группирповка по продавцу
order by
    income desc  -- сортировка по выручке в обратно порядке
...
from seller_avg as sa
    join overall_avg as oa
        on 1 = 1  -- связываем данные без условий
where
    sa.avg_income_raw < oa.glob_avg_incom
order by
    average_income;  --сортировка по возрастанию
...
from sales as s
    join employees as e
        on s.sales_person_id = e.employee_id  -- объединяем данные из таблиц
    join products as p
...
where
    age_category is not null  -- отсортировываем пустые ячейки
group by
    age_category  -- группируем по повзрастным категориям
order by
...
from sales as s
    join products as p
        on s.product_id = p.product_id  --объединяем таблицы чтобы узнать цену товара
group by
...
with sales_cte as (  -- используем временную таблицу для того чтобы подтянуть к каждой продаже клиента и цену
    select
        ...
        row_number() over (
            ...
            order by
                s.sale_date,
                s.sales_id  -- используем оконную функцию чтобы присвоить каждой строке порядковые номер
        ) as rn
    from sales as s
        join products as p
            on s.product_id = p.product_id  -- объединяем таблицы чтобы добавить цену к продаже
    where
        p.price = 0  -- акционная покупка
)
select
...
from sales_cte as sc
    join customers as c
        on sc.customer_id = c.customer_id  -- данные о покупателе
    join employees as e
        on sc.sales_person_id = e.employee_id  -- данные о продавце
where
